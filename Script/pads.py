...
for i in range(len(pad_names_4)):
    for j in range(4):
        print("PADDO pad_" + str(pad_names_4[i]) + str(j) + "(.PAD(" + str(pad_names_4[i]) + "[" + str(j) + "]), .Α(" + str(pad_names_4[i]) + "_w" + "[" + str(j) + "]), .VDD(VDD), .VSS(VSS));")

pad_names_36 = ['trace_data']

for i in range(len(pad_names_36)):
    for j in range(36):
        print("PADDO pad_" + str(pad_names_36[i]) + str(j) + "(.PAD(" + str(pad_names_36[i]) + "[" + str(j) + "]), .Α(" + str(pad_names_36[i]) + "_w" + "[" + str(j) + "]), .VDD(VDD), .VSS(VSS));")
